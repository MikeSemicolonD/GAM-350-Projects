...
public class ServerNetwork : UCNetwork
{
    ...
    public UnityEngine.UI.Text UITextServerLog, UITextServerStatus;

    // StreamWriter for logging to a file, if enabled
    public bool writeToLogFile = false;
    public bool writeToUnityDebugLog = false;
    ...
    void OnDestroy()
    {
        LogText(connection.Statistics.ToString());
    ...
    public void AddToArea(long aClientId, int aNewAreaId)
    {
        ...
        LogText("Client " + aClientId + " added to area " + aNewAreaId);
    ...
    public void AddObjectToArea(int aNetworkId, int aNewAreaId)
    {
        ...
        */
        ...
        SendMessage("AddedPlayerToArea", obj.networkId, SendMessageOptions.DontRequireReceiver);
    }
    ...
    NetworkObject InstantiateNetworkObject(string aPrefabName, Vector3 aPostion, Quaternion aOrientation, long aClientIdentifier, int aNetworkId, bool aSpawnedByServer, string aObjectJson, object aData = null)
    {
        ...
        */
        ...
        iod.data = aData;
        SendMessage("OnInstantiateNetworkObject", iod, SendMessageOptions.DontRequireReceiver);
    ...
    */
    ...
    override protected void HandleMessage_ConnectionApproval(NetIncomingMessage aMsg)
    {
        ...
        LogText("ConnectionRequest for " + data.id + ": " + data.ip);
    ...
    {
        LogText("ConnectionApproved for " + aClientId);
        ...
        LogText("ConnectionApproved called with unknown client " + aClientId);
    }
    public void ConnectionDenied(long aClientId)
    {
        LogText("ConnectionDenied for " + aClientId);
        for (int i = 0; i < pendingConnections.Count; i++)
        {
            if (pendingConnections[i].id == aClientId)
            {
        ...
        LogText("ConnectionDenied called with unknown client " + aClientId);
    }
    ...
    {
        LogText("HandleMessage_ConnectionLatencyUpdated is unimplemented");
    }
    ...
    void DestroyNetObject(int aObjectId, bool aObjectDead = true)
    {
        ...
        SendMessage("OnDestroyNetworkObject", aObjectId, SendMessageOptions.DontRequireReceiver);
    ...
    public void UpdateServerStatusUI(string message)
    {
        UITextServerStatus.text = message;
    }
    ...
    // Log some text
    public void LogText(string aLogEntry, bool UCNetworkLog = false)
    {
        if (writeToLogFile)
        {
        ...
        if(!UCNetworkLog)
        {
            UITextServerLog.text += aLogEntry+"\r\n";
        }
        else
        {
            UITextServerStatus.text += aLogEntry + "\r\n";
        }
    }
    ...
    void LogRPC(NetOutgoingMessage aMsg, UCNetwork.MessageReceiver aReceiver)
    {
        ...
        foreach (char c in argDef)
        {
            switch (c)
            {
                ...
                case 'v':
                    logMessage += new Vector3(aMsg.ReadFloat(), aMsg.ReadFloat(), aMsg.ReadFloat());
                    break;

                case 'q':
                    logMessage += new Quaternion(aMsg.ReadFloat(), aMsg.ReadFloat(), aMsg.ReadFloat(), aMsg.ReadFloat());
                    break;
    ...
    void LogRPC(NetIncomingMessage aMsg)
    {
        ...
        foreach (char c in argDef)
        {
            switch(c)
            {
                ...
                default:
                    LogText("Unhandled RPC parameter type " + c);
                    break;
            }
